...
class AjaxPublishView(AP):
    def ajax_save_reports(self):
        ...
        storage = getMultiAdapter((self.context, self.request), IPdfReportStorage)

        report_groups = []
        for pdf, html, csv_text, uids in zip(
            pdf_reports, html_reports, csv_reports, report_uids
        ):
            ...
            # store the report(s)
            objs = storage.store(
                pdf, html, uids, metadata=metadata, csv_text=csv_text, coa_num=coa_num
            )
        ...
        exit_urls = map(
            lambda reports: self.get_exit_url_for(reports, action=action), report_groups
        )
    ...
    def get_geochemistry_body(self, samples):
        ...
        eligible_analysis_services = sorted(
            [item for item in analysis_services_full_list if item.getSortKey()],
            key=lambda x: x.getSortKey(),
        )
        methods_list = ["Method"]
    ...
    def get_geochemistry_headers(self, samples, coa_num):
        ...
        headers.append(
            [
                ...
                lab_obj.getName(),
                lab_obj.getPhysicalAddress().get("city"),
            ]
        )
        headers.append(["\n"])
        headers.append(
            ["Country of Sample Origin", sample.Client.PostalAddress.get("country")]
        )
        headers.append(["\n"])
        batch_title = ""
        if sample.Batch:
    ...
    def create_field_sample_rows(self, grouped_analyses, field_analyses, sample_ids):

        if grouped_analyses.get("field"):
            for field_AS in grouped_analyses.get("field"):
                position_at_top = sample_ids.index(field_AS.get("sampleID")) - 1
                title = field_AS.get("title")
                field_analyses[title][position_at_top] = field_AS.get("result")
        return field_analyses

    def create_lab_sample_rows(self, grouped_analyses, lab_analyses, sample_ids):

        if grouped_analyses.get("lab"):
            for lab_AS in grouped_analyses.get("lab"):
                position_at_top = sample_ids.index(lab_AS.get("sampleID")) - 1
                title = lab_AS.get("title")
                if lab_analyses.get(title):
                    lab_analyses.get(title)[position_at_top] = lab_AS.get("result")
        return lab_analyses
    ...
    def get_zlabs_headers(self, samples, coa_num):
        ...
        headers.append(
            [
                ...
                sample.Client.title,
                "Client Contact",
                sample.Contact.title,
                "Client Contact Email Address",
                sample.Contact.EmailAddress,
            ]
        )
        headers.append(
            [
    ...
    def ajax_templates(self):
        ...
        return sorted(
            [item for item in templates if "bika" in item or "testit" in item]
        )
